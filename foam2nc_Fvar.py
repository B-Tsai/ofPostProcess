...
def readOf(case_path, t, of_name, bnd):
    ...
    ofdata = eval("fluidfoam." + readtype + "(case_path, t2str(t), '" + of_name + "', boundary='" + bnd + "')")
    return ofdata
...
for of_name in of_name_list: # Loop over all assigned OpenFOAM variables
    ...
    ncfile_path = os.path.join(nc_path, nc_name + '_' + bnd + '.nc')
    ...
    if not os.path.exists(ncfile_path):
        ...
        F = nc.Dataset(os.path.join(nc_path,'F_' + bnd + '.nc'), 'r')
        n = len(F.variables['Fx'])
        F.close()
    ...
    for it in t_new:
        ...
        if (idx >= len(t_old) and idx <= len(t_new)):
            ...
            ofdata = readOf(case_path, it, of_name, bnd)
